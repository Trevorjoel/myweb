
<div class= "sidebar_right">



   <br>



	<div class="nav">
		<ul id="ul_sides" >
			<li id="li_sides1">Featured Articles:</li>
			

			<li id="li_sides2"><a href="thejingles.php?id=2">The<br>Jingles</a>
			<a href="images/jingles.jpg"><img
					src="images/jingles.jpg" width="180" height="100"
					alt="ratmonkey" /></a>
			</li>

			<li id="li_sides2"><a href="gallery_aus.php?id=5">Australian Gallery</a>
			<a href="images/Trevor/broomebeach.jpg"><img
					src="images/Trevor/broomebeach.jpg" width="180" height="100"
					alt="ratmonkey" /></a>
			
			</li>
			<li id="li_sides4"><a href="english-for-real-life.php">English Lessons</a>
			<a href="images/erlbanner.jpg"><img
					src="images/erlbanner1.jpg" width="190" height="100"
					alt="ratmonkey" /></a>
			</li>


			<li id="li_sides5">Donate?
			<form class="donate_home" action="https://www.paypal.com/cgi-bin/webscr" method="post"
						target="_top">
						<input type="hidden" name="cmd" value="_s-xclick"> <input
							type="hidden" name="encrypted"
							value="-----BEGIN PKCS7----------END PKCS7-----
"> <input type="image"
							src="https://www.paypalobjects.com/en_AU/i/btn/btn_donateCC_LG.gif"
							border="0" name="submit"
							alt="PayPal — The safer, easier way to pay online."> <img alt=""
							border="0"
							src="https://www.paypalobjects.com/en_AU/i/scr/pixel.gif"
							width="1" height="1">
	
				</form>
			Or visit the partners below!
			</li>
		</ul>
	</div>
</div>
     <div></div>
<!--  
<a href="musicandart.php"><h4 id= h4sidebar_right style= "font-size: 110%">Music/Art</h4></a>
<a href="thejingles.php?id=2">The jingles</a><br>
<a href="gallery_aus.php?id=5">Bret and Sam</a>
<a href="travel.php"><h4 id= h4sidebar_right style= "font-size: 110%">Travels</a></h4>
<h4 id= h4sidebar_right style= "font-size: 110%"><a href="teach_learn.php">Teach/Learn</h4></a>
<a href="thejingles.php?id=2">Learn PHP</a>
<h4 id= h4sidebar_right style= "font-size: 110%"><a href="marketplace.php">Marketplace</a>
<h4 id= h4sidebar_right style= "font-size: 110%"><a href="index.php">Home Page</a></h4>
-->


